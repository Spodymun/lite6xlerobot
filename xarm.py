...
class Xarm(Robot):
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        self._arm = XArmAPI("10.77.77.200", is_radian=True)
        self._arm.connect()
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        # 1s Warmup: halte aktuelle Gelenkwinkel, ignoriere komische Start-Commands
        ...
        has_joints = all(k in action for k in joint_keys)
        ...
        if all(action.get(f"joint{i}.pos", None) == 0.0 for i in range(1, 7)):
            logger.warning("Blocking all-zero joint command (likely default init).")
            action.pop("joint1.pos", None)  # oder return ohne senden
            return action
