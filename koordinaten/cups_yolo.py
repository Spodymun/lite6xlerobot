#!/usr/bin/env python3
"""
Cup Detection via YOLO - Simplified version (with optional cup_warp)

Machine-readable output:
  CUP_MM x_mm y_mm conf

Beispiel:
  python3 cups_yolo.py --cam 3 --H H.npy --warp_npz cup_warp.npz --device cpu --once
"""
...
def main():
    ...
    ap.add_argument("--cup-class", type=str, default="cup",
                    help="YOLO class name to treat as cup")

    ap.add_argument("--device", type=str, default="cpu", choices=["cpu", "cuda"],
                    help="Force inference device. Use cpu if CUDA arch mismatch.")

    # Optional warp: mm_measured -> mm_true
    ap.add_argument("--warp_npz", type=str, default="",
                    help="Optional cup warp npz (cx,cy coeffs). If set, warp is applied after H.")

    args = ap.parse_args()

    print("INFO: q/ESC zum Beenden")
    print(f"INFO: cam={args.cam} weights={args.weights} device={args.device} conf={args.conf} class={args.cup_class}")
    print(f"INFO: H={args.H} (fixed)")

    # Load H (pixel->table mm)
    h_path = Path(args.H)
    ...
    while True:
        ...
        if best is not None:
            ...
            cv2.putText(vis, f"CUP table_mm=({table_xy[0]:.1f},{table_xy[1]:.1f})",
                        (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

            # Machine-readable output:
            print(f"CUP_MM {table_xy[0]:.1f} {table_xy[1]:.1f} {conf:.3f}")

            if args.once:
                break
        else:
            cv2.putText(vis, "No cup detection", (20, 70),
